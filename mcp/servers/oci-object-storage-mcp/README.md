...
- crear bucket:
  `py -3 mcp/servers/oci-object-storage-mcp/server.py --environment dev --command create-bucket --bucket-name bucket-landing-external --layer landing_external`
- cargar objeto:
  `py -3 mcp/servers/oci-object-storage-mcp/server.py --environment dev --command upload-object --bucket-name bucket-landing-external --source-file <ruta>`
